...
요약은 기본 2~3문장으로 만들며 매우 짧은 게시글은 1문장이 될 수 있습니다. 게시글당 저장 본문은 최대 20,000자·JSON 20KB, 모델 입력은 최대 12,000자이며 입력이 잘리면 화면에 `본문 일부 요약`으로 표시합니다. `GEMINI_MODEL`을 사용하고 일일 횟수 제한은 없습니다. 게시글 ID·본문 해시·모델/프롬프트 버전으로 30일 동안 재사용하며, 공유 DB 작업 선점에 실패하면 유료 호출을 하지 않습니다. 웹 종료 시 대기 작업을 취소하고 실행 중인 요약이 끝난 뒤 AI 연결을 닫습니다.
