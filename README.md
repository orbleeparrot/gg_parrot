...
| GET  | `/api/chat` | 오늘(KST) 채팅 목록·미확인 수, `before_id`로 이전 메시지 조회 |
| POST | `/api/chat` | 로그인 회원의 채팅 전송(`text`, 회원별 길이·전송 제한) |
| PUT  | `/api/chat/read` | 로그인 회원의 읽음 위치 저장(`last_seen_id`, 이전 값으로 되돌리지 않음) |
...
### E. 리더보드 채팅
- 누구나 읽을 수 있으며 작성은 로그인 회원만 가능합니다. 서버가 인증된 회원 ID와 이름으로 작성자를 결정하고, 내 메시지 표시도 회원 ID를 사용합니다. 기존 작성자 미확인 기록은 `이전 익명`으로 구분합니다.
- 3초 폴링, 300자 제한, **회원별 10초 5개** 전송 제한을 적용합니다. 전송 제한은 DB에 기록되어 같은 IP의 다른 회원과 공유하지 않습니다. React가 텍스트를 이스케이프하며 HTML을 실행하지 않습니다.
- NEW와 읽음 위치는 회원별로 관리합니다. 페이지 왕복·다중 탭에서는 브라우저 캐시를 유지하고 다른 기기와는 서버 저장값을 동기화합니다. 처음 방문했을 때의 기존 대화는 기준점으로 삼고, 이후 메시지는 채팅 하단이 실제로 보일 때 읽음 처리합니다.
- KST 당일 최신 200건부터 보여 주며 `이전 메시지 더 보기`로 같은 날의 이전 대화를 조회합니다. 미확인 수는 전체 당일 기록을 기준으로 집계합니다. 자정에는 전날 기록을 화면에서 제외하며 DB 기록과 읽음 ID를 초기화하지 않습니다.
- GET의 `seen_id`는 클라이언트 힌트를 반영한 집계 기준이고, `server_seen_id`는 실제 저장된 읽음 위치입니다. 읽음 저장 실패는 재시도하며, 늦은 조회 응답이 전송 성공 메시지를 제거하거나 읽음 기준을 되돌리지 않도록 병합합니다.
