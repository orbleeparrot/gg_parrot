...
def test_title_translation_keeps_partial_batch_without_paid_fanout(monkeypatch):
    ...
    assert [item["title"] for item in localized] == ["아비트럼 토큰 급등"]
...
def test_title_translation_reuses_shared_ai_client_and_model(monkeypatch):
    ...
    monkeypatch.setattr(news, "get_ai_client", fake_client)
    monkeypatch.setattr(news, "get_ai_runtime", lambda: Runtime())
...
def test_title_localization_logs_when_english_title_is_pending(monkeypatch, caplog):
...
def test_market_news_localizes_titles_before_summary_and_preserves_source_cache(monkeypatch):
    news._cache.clear()
    english = "Arbitrum token soars"
    korean = "아비트럼 토큰 급등"
    summarized = []
...
def test_market_summary_returns_plain_text_even_when_model_uses_markdown(monkeypatch):
    ...
    monkeypatch.setattr(
        news, "get_ai_client", lambda: type("Client", (), {"messages": Messages()})()
    )
    monkeypatch.setattr(news, "get_ai_runtime", lambda: Runtime())
...
def test_pure_english_title_that_cannot_be_translated_is_withheld(monkeypatch):
...
def test_failed_title_does_not_repeat_paid_call_on_refresh_and_accepts_durable_repair(monkeypatch):
    ...
    assert news._localize_coin_news_items([{"title": source}]) == []
    assert news._localize_coin_news_items([{"title": source}]) == []
    assert calls == [[source]]
