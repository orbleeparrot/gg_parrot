...
export default function ReportDialog({ open, targetType, targetId, label, onClose }) {
  ...
  return createPortal(
    <div className="scrim fixed inset-0 z-[90] grid place-items-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget && !busy) onClose?.(); }}>
      <form role="dialog" aria-modal="true" aria-labelledby={titleId} className="dialog confirm-dialog board-report" onSubmit={submit}>
        ...
        {done ? (
          ...
          <>
            ...
            <div className="board-report-reasons" role="radiogroup" aria-label="신고 사유">
              {REPORT_REASONS.map(([key, text]) => (
                <label key={key} className={`board-report-reason${reason === key ? " is-on" : ""}`}>
                  <input type="radio" name="reason" value={key} checked={reason === key} onChange={() => setReason(key)} />
                  <span>{text}</span>
                </label>
              ))}
            </div>
            <textarea value={detail} onChange={(e) => setDetail(e.target.value)} rows={2} maxLength={500} className="field board-report-detail" placeholder="덧붙일 말이 있으면 적어 주세요 (선택)" aria-label="덧붙일 말" />
