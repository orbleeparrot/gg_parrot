  ...
  return (
    <section className={`agent-result is-${outcome.tone}`} aria-label={`${session?.symbol || ""} 실행 결과`}>
      <div className="agent-result-head">
        ...
        <p className="agent-result-eyebrow">{outcome.eyebrow}</p>
      ...
      <div className="agent-result-pnl-block">
        ...
        <strong className={`agent-result-pnl num is-${outcome.pnl.tone}`}>{outcome.pnl.text}</strong>
      </div>
      ...
      <p className="agent-result-note">실현손익은 실행기가 보고한 누적값(USDT)이에요. 거래소 체결 내역과 대조해 확인하세요.</p>
      {onShowLog ? (
