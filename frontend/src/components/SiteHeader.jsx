        ...
        <div className="site-header-utility header-tools">
          {hasSidebar ? (
            <nav className="header-resources" aria-label="설치 및 사용 안내">
              <NavLink to="/runner/install" className="header-control header-resource t-label" aria-label="실행기 설치">
                <DownloadIcon /><span className="header-resource-label">실행기 설치</span>
                <span className="header-tooltip" aria-hidden="true">실행기 설치</span>
              </NavLink>
              <Link to={`/?${docsParams.toString()}`} state={pathname === "/" ? undefined : { helpReturnTo: pathname + search }} className="header-control header-resource t-label" aria-haspopup="dialog" aria-expanded={helpOpen} aria-current={helpOpen ? "page" : undefined} aria-label="사용법">
