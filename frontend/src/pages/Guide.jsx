...
const BASE_SECTIONS = [
  ...
  {
    id: "binance-api",
    title: "바이낸스 실거래 API 키 발급받기",
    text: "바이낸스 api 키 발급 실거래 실전 api management 시스템 생성 hmac secret key 시크릿 권한 편집 출금 금지 ip 제한 화이트리스트 90일 만료 2fa otp 현물 선물 실행기 러너 테스트넷 데모",
    body: (go) => (
      <>
        <P>
          껄무새 웹(백테스트·페이퍼·리더보드)은 실제 주문을 보내지 않아요. 실제로 주문을 내는 건 내
          PC에서 도는 <b>껄무새 실행기</b>뿐이고, 실행기가 바이낸스에 접속하려면 <b>API 키</b>가 필요해요.
          여기서는 <b>실거래(실제 자금) 키</b>를 만드는 순서를 정리해요.
        </P>
        <Note>
          <b>먼저 테스트넷부터.</b> 실행기는 기본값이 <b>테스트넷(가짜 자금)</b>이에요. 매크로가 며칠
          동안 의도대로 도는지 테스트넷에서 확인한 뒤에 실거래 키로 바꾸세요. 테스트넷 키 발급은
          실행기 다운로드 화면이 순서대로 안내해요.
        </Note>

        <H>1. API 관리 화면 열기</H>
        <Steps
          items={[
            "바이낸스에 로그인하고, 오른쪽 위 프로필 아이콘 → 계정(Account) → API 관리(API Management)로 들어가요.",
            "실거래 키는 KYC(본인 인증)와 2단계 인증(2FA)이 끝난 계정에서만 만들 수 있어요.",
            "선물(숏·레버리지) 매크로를 돌릴 거라면 선물 계정을 먼저 개설·활성화해 두세요. 그래야 선물 권한이 보여요.",
          ]}
        />
        ...
        <Steps
          items={[
            "‘API 키 생성(Create API)’을 누르고 ‘시스템 생성(System generated · HMAC)’을 선택해요.",
            "키 이름은 나중에 알아보게 지어요. 예: ggparrot-runner.",
            "2단계 인증(구글 OTP·이메일·SMS)을 통과하면 API Key와 Secret Key가 화면에 나와요.",
            "Secret Key는 이 화면을 닫으면 다시 볼 수 없어요. 비밀번호 관리자에 바로 저장하세요(스크린샷·메신저·메모장 금지).",
          ]}
        />

        <H>3. 권한을 최소로 잘라내기 (가장 중요)</H>
        <P>
          만든 키의 <b>권한 편집(Edit restrictions)</b>에서 필요한 것만 켜요. 이 한 단계가 사고가 났을 때
          피해 크기를 정해요.
        </P>
        <Params
          rows={[
            ["Enable Reading", "잔고·주문 조회. 켜 둬요(기본)."],
            ["Enable Spot & Margin Trading", "현물 매크로를 돌릴 때만 켜요."],
            ["Enable Futures", "선물(숏·레버리지) 매크로를 돌릴 때만 켜요."],
            ["Enable Withdrawals", "출금 권한. 껄무새 실행기는 출금 기능이 아예 없어요. 절대 켜지 마세요."],
            ["Symbol Whitelist", "가능하면 실제로 돌릴 종목만 허용해요."],
          ]}
        />
        <Note>
          <b>출금 권한(Enable Withdrawals)은 켜지 마세요.</b> 거래 권한만 있는 키는 유출돼도 자산을
          빼갈 수 없지만, 출금 권한이 있는 키는 계정을 통째로 잃을 수 있어요.
        </Note>

        <H>4. IP 제한 걸기</H>
        <Steps
          items={[
            "‘신뢰할 수 있는 IP만 허용(Restrict access to trusted IPs only)’을 선택해요.",
            "실행기를 돌릴 PC의 공인 IP를 넣어요. (검색창에 ‘내 아이피’를 치면 확인돼요)",
            "IP 제한을 걸지 않은 키는 바이낸스 정책상 90일 뒤 자동으로 만료돼요. 어느 날 갑자기 주문이 안 나가면 이걸 먼저 의심하세요.",
            "집 인터넷이 유동 IP라 자꾸 바뀐다면, IP가 바뀔 때마다 키 설정도 고쳐 줘야 해요.",
          ]}
        />
        ...
        <Steps
          items={[
            "껄무새 실행기를 열고 ①매크로 파일 ②실거래 여부 ③API 키·시크릿 ④회원 키를 넣어요.",
            "④회원 키는 껄무새 계정 연결용이라 거래소 API 키와 다른 값이에요. 실행기 다운로드 화면에서 복사해요.",
            "실행기 로그에 ‘연결 성공’이 뜨면 거래소 인증까지 끝난 거예요.",
            "처음에는 아주 작은 금액으로 시작해 주문이 의도대로 나가는지 눈으로 확인하세요.",
            "매크로 파일(.ggm.json)은 받은 그대로 넣어요. 메모장으로 고쳐서 넣으면 실행기와 내 에이전트에 '수정된 파일'로 표시되고, 웹 설정과 다르게 동작해도 껄무새 오류가 아니에요.",
            "실행 중 신호·주문·체결·오류는 내 에이전트의 '실행 로그' 탭에 남아요. 이상하면 그 로그를 보고 문의해 주세요.",
          ]}
        />
        <Tip>
        ...
        <Note>
          ...
          거래소 점검 중에도 포지션은 남아 있어요. 키가 샜다고 느끼면 API 관리에서 <b>즉시 삭제</b>하고
          새로 만드세요. 껄무새는 투자 자문을 제공하지 않아요.
        </Note>
        <P>
          레버리지·숏이 뭔지 아직 헷갈린다면{" "}
          <button onClick={() => go("long-short")} className="t-body font-bold text-slate-900 underline">
            롱/숏 설명
          </button>
          을 먼저 읽어보세요.
        </P>
  ...
  },
  {
    id: "leaderboard",
    title: "리더보드 · 포인트 · 언락",
    text: "리더보드 포인트 언락 등록 수익률 경쟁 왕관 판매 70% 마이페이지 AI 챌린지 초기화 방어중 연속 1등 2등 3등 이월 방어전 퀘스트 일일 퀘스트 오늘의 퀘스트",
    body: (
      <>
        <P>
        ...
        <H>1·2·3등은 초기화 없이 그대로 이어져요 (방어전)</H>
        <P>
          자정에 보드가 비워질 때 <b>상위 3등은 지워지지 않고 다음 날 보드로 그대로 넘어가요</b>.
          수익률도 초기화하지 않아요 — <b>등록한 그 시점부터의 누적 수익률</b>을 계속 들고
          가면서 새로 올라온 매크로들과 겨뤄요. 그날도 3등 안에 들면 또 이어지고, 이렇게 며칠을
          버틴 매크로에는 <b>"N일째 순위권 방어중"</b> 표시가 붙어요.
        </P>
        <P>
          매크로가 <b>제 조건대로 끝난 경우</b>(목표 익절선 도달, 최대 손실선, 보유 시간 만료)에도
          그 <b>종료 시점의 수익률이 그대로 순위에 남아요</b>. 언제 들어갔는지도, 언제 끝냈는지도
          매크로가 정한 것이니 그것까지가 실력이라고 봐요. 리더보드는 <b>매크로 하나의 순수한
          수익률 경쟁</b>이에요.
        </P>
        <P>
          그래서 상위권은 "오늘 운이 좋았던 매크로"가 아니라 "며칠째 살아남은 매크로"를 함께
          보여줘요. 4등 아래는 초기화와 함께 보드에서 내려가요(내 매크로는 마이페이지에 그대로 남아요).
        </P>
        <H>포인트와 언락</H>
        <Steps
          items={[
            ...
            "매크로를 안 올려도 돼요 — 매일 자정(KST)에 열리는 오늘의 퀘스트(백테스트 1회·페이퍼 트레이딩 1회·10자 이상 댓글 1개, 각 10P)로 하루 최대 30P를 벌어요. 마이페이지에서 확인해요.",
  ...
  },
  {
    id: "faq",
    title: "코린이 자주 헷갈리는 것",
    text: "FAQ 자주 묻는 질문 실거래 되나요 왜 안 사요 조건 진입 청산 리플레이 라이브 종목 심볼 BTCUSDT 헷갈림 포트폴리오 멀티종목 복수 종목 여러 종목 자금 분할 총합",
    body: (
      <>
        <H>실제로 돈이 움직이나요?</H>
        <P>
        ...
        <P>거래쌍 형식이에요. 예: 비트코인은 <b>BTCUSDT</b>, 이더리움은 <b>ETHUSDT</b>. 쉼표로 여러 개를 넣으면(<b>BTCUSDT, ETHUSDT</b>) 자금을 균등 분할하는 <b>포트폴리오</b>가 돼요(최대 5개).</P>
        <H>껄무새에게 물어볼까? 는 투자 추천인가요?</H>
        <P>아니에요. 고른 종목과 조건으로 여러 조합을 과거 데이터에 돌려 본 뒤 <b>상위 3개</b>를 보여 주는 탐색 도구예요. 종목은 직접 고른 것만 쓰고, 숫자는 전부 백테스트 결과예요. <b>투자 권유가 아니고</b> 과거 성과가 미래 수익을 보장하지 않아요. <b>하루 5번</b>까지 물어볼 수 있어요.</P>
        <H>전략방은 뭔가요? 입장료는 환불되나요?</H>
        <P>리더보드 채팅에서 회원이 만드는 최대 10명 소그룹 대화방이에요. 만들 때 100포인트, 들어갈 때 방장이 정한 입장료(0~300포인트)를 내고, 입장료의 70%는 방장에게 가요. 7일 뒤 자동으로 끝나고 환불은 없어요. 전략방 대화는 투자 조언이 아니며, 매수·매도 권유나 수익 보장 발언은 금지돼요.</P>
        <H>종목을 여러 개 고르면 매매가 어떻게 되나요?</H>
        <P>한 매크로 안에서 <b>종목마다 따로</b> 돌아요. 종목끼리 자금을 섞거나 서로 조건을 보지 않아요.</P>
        <Steps
          items={[
            "자금 분할: 시작 자금을 종목 수로 똑같이 나눠요. 예: 300만 USDT · 3종목이면 종목당 100만 USDT.",
            "각각 독립: 같은 규칙(익절·손절·지표 조건)이 종목마다 그 종목의 시세로 따로 판정돼요. BTC 조건이 맞으면 BTC만 사고, ETH는 ETH 시세로 따로 봐요. 한 종목의 손실·청산이 다른 종목의 자금을 건드리지 않아요.",
            "결과는 총합: 수익률·평가금액·청산 횟수는 종목별 결과를 합친 값이에요. 백테스트엔 '종목별 성과' 표가, 페이퍼 트레이딩엔 총합 아래 종목별 수익률과 종목이 표시된 매매 로그가 같이 나와요.",
            "리더보드: 총합 수익률로 겨뤄요(카드에는 첫 번째 종목이 대표로 보여요).",
            "실행기(실거래)는 아직 한 종목만: 매크로 파일을 실행기에 넣으면 첫 번째 종목으로만 돌아요. 여러 종목을 실제로 돌리려면 종목별로 매크로를 따로 만들어 각각 실행해요.",
          ]}
        />
        <H>실시간 / 데모 리플레이 차이는?</H>
