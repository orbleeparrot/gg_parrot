...
const { setAuth, updateAuthUser, clearAuth } = await import("../src/lib/auth.js");
const { leaderboardCacheVersion } = await import("../src/lib/cacheEvents.js");

function accountFixture(t) {
...
test("public GETs omit credentials while private API reads and writes remain no-store", async (t) => {
  accountFixture(t);
  const requests = [];
