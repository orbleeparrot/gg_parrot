...
## v9 — 키 기억하기 · 새 디자인
- **이 PC에 키 기억하기**: 체크하고 시작하면 API 키·시크릿·회원 키를 `%LOCALAPPDATA%\GGParrot\credentials.dat`에
  Windows DPAPI(현재 사용자 범위)로 암호화해 둡니다. 같은 Windows 계정에서만 풀리고, 다른 PC로 복사하면 열리지 않아요.
  서버로는 보내지 않습니다. 체크를 풀고 시작하거나 "저장된 키 지우기"를 누르면 파일을 지웁니다.
- 디자인: 껄무새 웹과 같은 톤(노랑 포인트·흰 카드), 로그는 종류별 색·아이콘(🔔 신호 · ▶ 주문 · ✓ 체결 · ⚠ 오류 · ■ 종료).
  기능 변경은 없습니다.
